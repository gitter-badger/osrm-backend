    ...
    : public BasicRoutingInterface<DataFacadeT, ShortestPathRouting<DataFacadeT>>
{
  ...
  public:
                         ...
                         std::vector<NodeID> &leg_packed_path) const
    {
        ...
        const auto forceLoop =
            [&source_phantom, &target_phantom](bool forward)
        {
            if (forward)
        ...
        // We force a detour by inserting "virtual vias", which means we search a path
        ...
        // ------^
        void
        SearchLoop(QueryHeap & forward_heap, QueryHeap & reverse_heap,
                   const bool search_forward_node, const bool search_reverse_node,
                   const PhantomNode &source_phantom, const PhantomNode &target_phantom,
                   const int total_distance_to_forward, const int total_distance_to_reverse,
                   int &new_total_distance_to_forward, int &new_total_distance_to_reverse,
                   ...
                   std::vector<NodeID> &leg_packed_path_reverse) const
        {
            ...
            const auto getOffsetSum = [&source_phantom, &target_phantom,
                                       SEARCH_DIRECTION_FORWARD](bool search_direction) -> int
            {
            ...
            {
                int loop_distance;
                const auto node_id = source_phantom.forward_node_id;
                super::SearchLoop(node_id, SEARCH_DIRECTION_FORWARD,
                                  total_distance_to_forward +
                                      getOffsetSum(SEARCH_DIRECTION_FORWARD),
                                  loop_distance, leg_packed_path_forward);
                ...
                if (new_total_distance_to_forward >= loop_distance)
                {
                    new_total_distance_to_forward = loop_distance;
                    leg_packed_path_forward.clear();
            ...
            {
                const auto node_id = source_phantom.reverse_node_id;
                int loop_distance;
                super::SearchLoop(node_id, SEARCH_DIRECTION_REVERSE,
                                  total_distance_to_reverse +
                                      getOffsetSum(SEARCH_DIRECTION_REVERSE),
                                  loop_distance, leg_packed_path_reverse);
                forward_heap.Clear();
                reverse_heap.Clear();
                ...
                if (loop_distance < new_total_distance_to_reverse)
                {
                    new_total_distance_to_reverse = loop_distance;
                    leg_packed_path_reverse.clear();
        ...
        void Search(QueryHeap & forward_heap, QueryHeap & reverse_heap,
                    const bool search_from_forward_node, const bool search_from_reverse_node,
                    const bool search_to_forward_node, const bool search_to_reverse_node,
                    const PhantomNode &source_phantom, const PhantomNode &target_phantom,
                    const int total_distance_to_forward, const int total_distance_to_reverse,
                    int &new_total_distance_to_forward, int &new_total_distance_to_reverse,
                    std::vector<NodeID> &leg_packed_path_forward,
            ...
            if (search_to_forward_node)
            {
                ...
                if (search_from_forward_node)
                {
                    std::cout << "\tFW: " << source_phantom.forward_node_id
                              << " Of: " << -source_phantom.GetForwardWeightPlusOffset()
                              << std::endl;
                    forward_heap.Insert(source_phantom.forward_node_id,
                                        total_distance_to_forward -
                                            source_phantom.GetForwardWeightPlusOffset(),
                ...
                if (search_from_reverse_node)
                {
                    std::cout << "\tFW: " << source_phantom.reverse_node_id
                              << " Of: " << -source_phantom.GetReverseWeightPlusOffset()
                              << std::endl;
                    forward_heap.Insert(source_phantom.reverse_node_id,
                                        total_distance_to_reverse -
                                            source_phantom.GetReverseWeightPlusOffset(),
        ...
        void UnpackLegs(const std::vector<PhantomNodes> &phantom_nodes_vector,
                        ...
                        const int shortest_path_length, InternalRouteResult &raw_route_data) const
        {
            ...
            for (const auto current_leg : util::irange<std::size_t>(0, packed_leg_begin.size() - 1))
            {
                ...
                raw_route_data.source_traversed_in_reverse.push_back(
                    (*leg_begin !=
                     phantom_nodes_vector[current_leg].source_phantom.forward_node_id));
